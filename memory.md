...
- Local commit `193029b8` was pushed to `fork/feat/codex-local-model-routing`. Official upstream issue filed as `farion1231/cc-switch#4349` proposing a separate Codex Desktop history visibility repair flow; do not open an upstream PR from the fork feature branch because it carries CCSwitchMulti-specific history. For a PR, start a clean branch from `origin/main` and port only the accepted repair scope.

## 2026-06-17 CCSwitchMulti v3.16.3-2 release

- Published `https://github.com/BigStrongSun/cc-switch/releases/tag/v3.16.3-2` from annotated tag `v3.16.3-2`; tag object `6be5c3fd82a368f5bf130fb4b22a9d56d105e526` dereferences to release commit `90fb89456fcb98e7d61d5b17c3e5246e50814b52`.
- This release contains the unified/Desktop history repair integration from `193029b8` plus the version bump `90fb8945`; a post-release memory-only commit may sit after the tag on `feat/codex-local-model-routing`.
- Local clean upload staging directory: `C:\Users\sunda\Documents\LLMservice\ccswitchmulti-release-v3.16.3-2\upload`. Do not upload from the default `最新版ccswitchmulti` folder for this release because that folder retained older raw-exe aliases.
- Uploaded 10 assets: `CCSwitchMulti_3.16.3-2_x64-setup.exe`, setup `.sig`, `CCSwitchMulti_3.16.3-2_x64-portable.zip`, `CCSwitchMulti_3.16.3-2_x64.exe`, `CodexHistoryTool_3.16.3-2.zip`, `latest.json`, `README.md`, `linux-build-note.md`, `macos-build-note.md`, and `SHA256SUMS-v3.16.3-2.txt`.
- Key SHA256 values: setup `8319D6D2468B5762F4F6D6E0F3758927C8AFCB67937D36A630BE85DC14FF33F5`; setup sig `693E378D533A461445483B715DF49C85791AE8BB940061F753D86655F1538247`; portable zip `50F708BDE40C885DEA2EDD988F34177B3DA1374C50A74DC6EAC1993B4DCEDD2F`; raw exe `3BDDABBE4B46BA33900544E66E1C65304CD95B3A38858D9FCEB42162182FBD63`; history tool zip `86325972DEC974382C1FDAC05BF1F7B0522FE1924BB5122C7E1E7D2EF78C7C9D`; `latest.json` `D9FC276B950802BDB3350B21851D3E5FE72B01255C97B629AF1B0B8F429FC64B`.
- Verification passed before and during release: `pnpm typecheck`; `cargo fmt --manifest-path src-tauri\Cargo.toml --check`; `cargo check --manifest-path src-tauri\Cargo.toml --lib`; `cargo test --manifest-path src-tauri\Cargo.toml codex_history_migration --lib`; `cargo test --manifest-path src-tauri\Cargo.toml codex_chat_history --lib`; `python -m py_compile scripts\codex-history-tool\codex_history_tool.py`; post-commit local release pipeline; versioned `-SkipBuild` export; GitHub release asset digest checks. Existing `commands/misc.rs` dead-code warnings remain.
...
## 2026-06-18 CCSwitchMulti v3.16.3-5 release and upstream PR

- Local fix release version is `3.16.3-5`. The previous `v3.16.3-3` and `v3.16.3-4` tags/releases were already used, so do not reuse them for future packaging.
- Published fork release: `https://github.com/BigStrongSun/cc-switch/releases/tag/v3.16.3-5`. It is the GitHub Latest release and includes Windows setup exe/signature, portable zip, raw exe, `latest.json`, `CodexHistoryTool_3.16.3-5.zip`, platform build notes, README, and `SHA256SUMS-v3.16.3-5.txt`.
- The macOS DMG that had been mistakenly published under `v3.16.3-4` was copied into `v3.16.3-2` as `CCSwitchMulti-v3.16.3-2-macOS-aarch64.dmg` with SHA256 `4C652002D78E2FA6CCBAAF4C5E7EDDE8022A35C90D6C2671A79CF99034913320`, and `SHA256SUMS-v3.16.3-2.txt` was updated. It is manual-download only because no Tauri updater signature was available.
- Official upstream `farion1231/cc-switch` had the same restore bug. Existing issue `farion1231/cc-switch#4254` matched the root cause, so no duplicate issue was opened.
- Upstream PR opened: `https://github.com/farion1231/cc-switch/pull/4381` from branch `codex/upstream-preserve-codex-live-settings`. The PR adds a minimal live/provider config merge helper in `src-tauri/src/codex_config.rs`, calls it from `write_codex_live_verbatim`, and adds `codex_restore_from_backup_preserves_live_desktop_settings`.
- Upstream PR verification passed: `cargo fmt --manifest-path src-tauri\Cargo.toml --check`; `cargo test --manifest-path src-tauri\Cargo.toml codex_restore_from_backup --lib -- --nocapture`; `cargo test --manifest-path src-tauri\Cargo.toml codex_restore_empty_auth_backup_still_projects_inline_catalog --lib -- --nocapture`; `cargo check --manifest-path src-tauri\Cargo.toml --lib`. Remaining warnings were pre-existing `settings.rs` unused import and `commands/misc.rs` dead_code warnings.
